...
def players():
    ...
    for idx, row in enumerate(player_list, start=1):
        ...
        table_data.append([idx, render_link(f"/players/{row[0]}", row[1])] + list(row)[2:])
...
def add_player():
    ...
    form = '\n'.join([
        ...
        render_row(render_label('rating', "Рейтинг", {'class': 'col-25'}),
                   render_input('number', {'name': 'rating', 'min': '100', 'max': '3000', 'step': '0.001'},
                                {'class': 'col-75'}),
                   {'class': 'row'}),
        ...
        render_row(render_label('national_rank_id', "Розряд", {'class': 'col-25'}),
                   render_select('national_rank_id', NationalRank.info(), div_attrs={'class': 'col-75'}),
                   {'class': 'row'}),
